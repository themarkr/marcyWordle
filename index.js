window.addEventListener('DOMContentLoaded', () => {
    ...
    function pickWord() {
        const wordSize = document.getElementById('word-length').value
        const filteredWordBank = masterWordBank.filter(word => word.length === wordSize);
        const bankSize = filteredWordBank.length;
        const randomIndex = Math.floor(Math.random() * bankSize)
        return bankSize[randomIndex];
    }
    ...
    function updateTiles(totalAttempts) {
        ...
        switch (totalAttempts) {
            ...
            case 6:
                // let i = 0;
                row6Tiles.forEach(tile => {
                        tile.innerText = guess[i];
                        // changeColor(tile);
                        i++;
                    })
                    // left off at creating the change color function
        }
    }
